...
app.get("/*", (req, res, next) => {
  ...
  superagent
    .get(
      `http://www.dongqiudi.com/data?competition=${
        competition[team]
      }&type=${_type}`
    )
          ...
          if (ele) {
            $(ele).each((index, element) => {
              ...
              switch (type) {
                ...
                case "mvp":
                  items.push({
                    team: escaper.unescape(
                      removeHTMLTag(elm.find(".team").html())
                    ),
                    rank: escaper.unescape(
                      removeHTMLTag(elm.find(".rank").html())
                    ),
                    mvp: escaper.unescape(
                      removeHTMLTag(elm.find(".player").html())
                    ),
                    stat: escaper.unescape(
                      removeHTMLTag(elm.find(".stat").html())
                    ),
                    img: elm.find("img").attr("src")
                  });
                  break;
